...
import { createAdminMenuItem, updateAdminMenuItem, deleteAdminMenuItem } from '../../api/adminApi';
...
const AdminMenu = ({ token }) => {
  ...
  const fetchRestaurantsList = async () => {
    ...
    try {
      ...
      console.error('Failed to load restaurants for selection');
    }
  };
  ...
  const openCreate = () => {
    ...
    setFormData({
      name: '', restaurant: '', category: '', description: '', price: '', discount: '', image: '', isVeg: false, isAvailable: true,
    });
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      if (selectedItem) {
        await updateAdminMenuItem(selectedItem._id, payload, token);
      } else {
        await createAdminMenuItem(payload, token);
      }
  ...
  const handleDelete = async () => {
    setActionLoading(true);
    try {
      await deleteAdminMenuItem(selectedItem._id, token);
  ...
  const getRestaurantName = (restaurantData) => {
    if (!restaurantData) return 'Unknown';
    if (typeof restaurantData === 'string') {
      const found = restaurants.find(r => r._id === restaurantData);
      return found ? found.name : 'Unknown';
    }
    return restaurantData.name || 'Unknown';
  };
  ...
  return (
    <div className="bg-white rounded-[32px] p-8 border border-outline-variant/20 shadow-premium">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h2 className="font-display text-headline-sm font-black text-on-surface">Menu Management</h2>
      ...
      {menuItems.length === 0 ? (
        <div className="text-center py-12 text-on-surface-variant font-medium">No menu items found.</div>
      ) : (
          ...
          <div className="space-y-4 text-body-md text-on-surface">
            <div className="flex items-center gap-4 mb-6">
              <div className="w-16 h-16 rounded-2xl overflow-hidden border-2 border-primary/20 bg-surface-container flex items-center justify-center text-primary">
                {selectedItem.image ? (
                  <img src={selectedItem.image} alt="Cover" className="w-full h-full object-cover" />
                ) : (
                  <Utensils size={32} />
                )}
              </div>
            ...
            <div className="grid grid-cols-2 gap-4 border-t border-outline-variant/20 pt-4">
              ...
              <div>
                <p className="text-label-sm text-on-surface-variant uppercase font-bold">Restaurant</p>
                <p className="font-medium">{getRestaurantName(selectedItem.restaurant)}</p>
              </div>
              <div>
      ...
      {/* Form Modal */}
      <Modal isOpen={formModalOpen} onClose={() => setFormModalOpen(false)} title={selectedItem ? "Edit Menu Item" : "Add Menu Item"}>
        <form onSubmit={handleFormSubmit} className="space-y-4 max-h-[60vh] overflow-y-auto px-1">
          ...
          <div className="flex flex-col gap-2">
            <label className="text-label-lg font-bold text-on-surface-variant">Restaurant</label>
            <select 
          ...
          <div className="grid grid-cols-2 gap-4">
            <Input label="Category" placeholder="e.g. Starters, Main Course" value={formData.category} onChange={(e) => setFormData({...formData, category: e.target.value})} required />
            <Input label="Image URL" value={formData.image} onChange={(e) => setFormData({...formData, image: e.target.value})} />
          </div>

          <div className="grid grid-cols-2 gap-4">
            <Input label="Price (₹)" type="number" min="0" value={formData.price} onChange={(e) => setFormData({...formData, price: e.target.value})} required />
          ...
          <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-outline-variant/30">
            ...
            <Button type="submit" variant="primary" loading={actionLoading} disabled={actionLoading}>
              {selectedItem ? 'Save Changes' : 'Create Menu Item'}
            </Button>
          </div>
        </form>
      </Modal>
        ...
        {selectedItem && (
          <div className="text-center py-4">
            ...
            <p className="text-on-surface-variant text-body-md mb-8">
              This will permanently delete <strong>{selectedItem.name}</strong> from the menu.
            </p>
